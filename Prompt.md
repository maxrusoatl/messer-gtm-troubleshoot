...
Repo inputs (use whatever exists under original-data/)
If a named file is missing, proceed with available files and list the missing item under Missing Source Requests.

Core inputs (use the latest match for each type):
- wGTM export JSON: `original-data/GTM-*_workspace*.json` with usageContext WEB
- sGTM export JSON: `original-data/GTM-*_workspace*.json` with usageContext SERVER
- Tag Assistant wGTM: `original-data/tag_assistant_*messerattach*_*.json` (web container)
- Tag Assistant sGTM: `original-data/tag_assistant_sgtm_*messerattach*_*.json`
- DataLayer samples: any `original-data/*datalayer*.json`, `original-data/*data layer*.json`, and console logs like `original-data/*console*.md` if present

Optional inputs (if present):
- Stape logs: `original-data/*stape*logs*.csv`
- Cloudflare Worker code: `original-data/*worker*metrics*.js`
- Cloudflare/edge notes: `original-data/*cloudflare*.md`
- WP Rocket settings: `original-data/*rocket*.md`
- Stape plugin settings: `original-data/STAPE_SGTM_PLUGIN_SETTINGS.md`
- Unique Event ID template: `original-data/*unique*id*template*.js`
- Prior reports (context only, not proof): `original-data/*report*.md`
