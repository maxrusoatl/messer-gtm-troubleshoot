Full Audit Report (evidence set: 2026-01-08 capture + current exports)
...
- user_id pipeline is partially broken: DataLayer uses top-level user_id and wGTM reads user_id, but sGTM Variable/EDV - user_id expects user_data.user_id and samples show empty user_data; server-side user_id is not observed. (Evidence: `original-data/new begin checkout data layer.json`, `original-data/tag_assistant_messerattach_com_2026_01_08.json`, `original-data/GTM-MNRP4PF_workspace252.json`, `original-data/GTM-K3CQBMZ9_workspace39.json`)
- event_id is configured in Data Tag custom_data via Unique Event ID, but is not present in DataLayer samples and cannot be verified end-to-end without /metrics or sGTM preview evidence. (Evidence: `original-data/GTM-MNRP4PF_workspace252.json`, `original-data/new view_iteam data layer.json`, `original-data/tag_assistant_sgtm_messerattach_com_2026_01_08.json`)
- Ecommerce payloads exist but with data-quality gaps: value/price/cart_total are strings, user_data is empty, ecomm_pagetype is basket on cart/checkout samples, and item_brand coverage beyond samples is not determinable. (Evidence: `original-data/new cart view.json`, `original-data/new begin checkout data layer.json`, `original-data/new view_iteam data layer.json`, `original-data/tag_assistant_messerattach_com_2026_01_08.json`)
- Consent enforcement is not configured at the tag level in either layer (wGTM Tag/Data Tag consentStatus NOT_NEEDED; sGTM tags consentStatus NOT_SET). (Evidence: `original-data/GTM-MNRP4PF_workspace252.json`, `original-data/GTM-K3CQBMZ9_workspace39.json`)
...
- wGTM container GTM-MNRP4PF exportTime not provided in export; containerVersionId 0. (Evidence: `original-data/GTM-MNRP4PF_workspace252.json`)
- sGTM container GTM-K3CQBMZ9 exportTime not provided in export; containerVersionId 0. (Evidence: `original-data/GTM-K3CQBMZ9_workspace39.json`)

ID / Key | Template | Where used
GT-NS9Z5FWG | wGTM Tag/Google Tag | server_container_url=https://messerattach.com/metrics; Tag Assistant shows send_to on form_start/form_submit/scroll only. (Evidence: `original-data/GTM-MNRP4PF_workspace252.json`, `original-data/tag_assistant_messerattach_com_2026_01_08.json`)
G-YCL5FGZNCV | GA4 Measurement ID | wGTM Variable/GA4 - ID; sGTM Tag/GA4 Advanced and sGTM Tag/GA4 ADV add_to_cart measurementId. (Evidence: `original-data/GTM-MNRP4PF_workspace252.json`, `original-data/GTM-K3CQBMZ9_workspace39.json`)
767740215 | Google Ads Conversion ID | wGTM Tag/google_ads_calls + google_ads_800calls; sGTM Tag/Google Ads Conversion Tracking + Remarketing. (Evidence: `original-data/GTM-MNRP4PF_workspace252.json`, `original-data/GTM-K3CQBMZ9_workspace39.json`)
...
ZLAFCNKDkusYELeSi-4C, WrSjCJnAiesYELeSi-4C | Google Ads call labels | wGTM Tag/google_ads_calls + google_ads_800calls labels. (Evidence: `original-data/GTM-MNRP4PF_workspace252.json`)
wGTM Tag/Data Tag endpoint | Stape Data Tag template | gtm_server_domain https://messerattach.com/metrics, request_path /data, event_name_custom {{Event page_view Override}}, add_consent_state true. (Evidence: `original-data/GTM-MNRP4PF_workspace252.json`)
...
Event-name normalization | wGTM Variable/Event page_view Override | gtm.dom and gtm.historyChange -> page_view; gtm.scrollDepth -> scroll. (Evidence: `original-data/GTM-MNRP4PF_workspace252.json`)

## Event Chain Results Table
Join key:
- Primary: event_id (Data Tag custom_data uses Unique Event ID; not present in DataLayer samples and not verifiable end-to-end without /metrics + sGTM preview). (Evidence: `original-data/GTM-MNRP4PF_workspace252.json`, `original-data/new view_iteam data layer.json`)
...
- DataLayer schema: value/price/cart_total are strings; fix by emitting numeric values before pushing to dataLayer. (Evidence: `original-data/new cart view.json`, `original-data/new begin checkout data layer.json`)
- DataLayer identity: user_id is top-level while sGTM Variable/EDV - user_id reads user_data.user_id; fix by aligning the key path (push user_data.user_id or update EDV - user_id). (Evidence: `original-data/new begin checkout data layer.json`, `original-data/GTM-K3CQBMZ9_workspace39.json`)
...
- wGTM consent gating: wGTM Tag/Data Tag consentStatus is NOT_NEEDED; fix by applying consent requirements at tag level before forwarding. (Evidence: `original-data/GTM-MNRP4PF_workspace252.json`)
...
- gtag classification: Not determinable from provided inputs (no network evidence; Tag Assistant shows send_to on form_start/form_submit/scroll only). (Evidence: `original-data/GTM-MNRP4PF_workspace252.json`, `original-data/tag_assistant_messerattach_com_2026_01_08.json`)
- Configured send paths:
  - wGTM Tag/Data Tag -> https://messerattach.com/metrics/data (fires on ecommerce events). (Evidence: `original-data/GTM-MNRP4PF_workspace252.json`, `original-data/tag_assistant_messerattach_com_2026_01_08.json`)
  ...
  - wGTM Tag/Google Tag GT-NS9Z5FWG configured with server_container_url /metrics; Tag Assistant shows send_to on form_start/form_submit/scroll only. (Evidence: `original-data/GTM-MNRP4PF_workspace252.json`, `original-data/tag_assistant_messerattach_com_2026_01_08.json`)
...
- Keep / Disable / Move server-side (policy):
  - Keep: wGTM Tag/Data Tag as the SOP sender once sGTM ingestion is confirmed. (Evidence: `original-data/GTM-MNRP4PF_workspace252.json`)
  ...
  - Keep client-side: wGTM Tag/ms_uet_view_item, ms_uet_add_to_cart, ms_uet_checkout, ms_uet_purchase must remain client-side; document as exceptions. (Evidence: `original-data/tag_assistant_messerattach_com_2026_01_08.json`)
- Deduplication rule: event_id must be consistent across any hybrid path; current event_id presence is not verifiable end-to-end. (Evidence: `original-data/GTM-MNRP4PF_workspace252.json`, `original-data/tag_assistant_messerattach_com_2026_01_08.json`, `original-data/tag_assistant_sgtm_messerattach_com_2026_01_08.json`)
...
- wGTM Tag/Data Tag consentStatus is NOT_NEEDED and sGTM tags consentStatus are NOT_SET. (Evidence: `original-data/GTM-MNRP4PF_workspace252.json`, `original-data/GTM-K3CQBMZ9_workspace39.json`)
- DataLayer user_data is empty at begin_checkout. (Evidence: `original-data/new begin checkout data layer.json`)
...
- Critical | Effort: S | Change: Align user_id mapping (update sGTM EDV - user_id keyPath to user_id or push user_data.user_id in DataLayer). | Validation: sGTM eventData includes user_id. (Evidence: `original-data/new begin checkout data layer.json`, `original-data/GTM-K3CQBMZ9_workspace39.json`)
- High | Effort: M | Change: Normalize value/price/cart_total to numbers; verify item_brand coverage beyond samples. | Validation: DataLayer samples show numeric values and item_brand coverage meets requirements. (Evidence: `original-data/new cart view.json`, `original-data/new begin checkout data layer.json`)
- High | Effort: S | Change: Apply consent requirements to wGTM Tag/Data Tag and sGTM tags. | Validation: tags do not fire when consent denied. (Evidence: `original-data/GTM-MNRP4PF_workspace252.json`, `original-data/GTM-K3CQBMZ9_workspace39.json`)
- Medium | Effort: S | Change: Remove or scope sGTM Tag/GA4 ADV add_to_cart to avoid duplication with sGTM Tag/GA4 Advanced. | Validation: only one GA4 server tag fires for add_to_cart. (Evidence: `original-data/GTM-K3CQBMZ9_workspace39.json`)
